...
const Chart = (props: any) => {
  ...
  useEffect(() => {
    generateChartData(props.stationname).then((response: any) => {
      setChartData(response);
    });

    generateTotalProgress(props.stationname).then((response: any) => {
      setProgress(response);
    });

    zoomToLayer(stationStructureLayer);
  }, [props.stationname]);
  ...
  useEffect(() => {
    ...
    function makeSeries(name: any, fieldName: any) {
      ...
      series.columns.template.events.on('click', (ev) => {
        ...
        const find = statioinStructureTypeChart.find((emp: any) => emp.category === categorySelect);
        ...
        view.when(() => {
          view.whenLayerView(stationStructureLayer).then((layerView: any) => {
    ...
    <div>
      <div className="totalProgressDiv">
        ...
        <img
          ...
          height={'35%'}
          width={'35%'}
          style={{ paddingLeft: '0px', margin: 'auto' }}
        />
      </div>
      <div
        id={chartID}
