...
export async function generateChartData(stationname: any) {
  ...
  // Query
  var query = stationStructureLayer.createQuery();
  query.outStatistics = [
    total_dwall_incomp,
    total_dwall_comp,
    total_dwall_delay,
...
export async function generateTotalProgress(stationname: any) {
  var total_structures_number = new StatisticDefinition({
    onStatisticField: 'Status',
    outStatisticFieldName: 'total_structures_number',
    statisticType: 'count',
  });

  var total_structures_comp = new StatisticDefinition({
...
export async function timeSeriesChartData(stationname: any) {
  ...
  if (!stationname) {
    ...
    // eslint-disable-next-line no-useless-concat
    query.where = 'end_date IS NOT NULL' + ' AND ' + "Station1 = '" + stationname + "'";
  }
